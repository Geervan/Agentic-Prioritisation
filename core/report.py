...
def generate_html_report(testcases, final_order, explanations, dataset_name: str, validation_result=None):
    ...
    if validation_result:
        ...
        <table>
            ...
            <tr>
                <td><strong>Lift Factor</strong></td>
                <td><strong>{validation_result.get('lift_factor', 1.0)}x</strong></td>
                <td>[How many times better (faster) the agent is vs random baseline]</td>
            </tr>
        """

        # Add Precision rows
        precision_at_k = validation_result.get("precision_at_k", {})
        if precision_at_k:
             for k in sorted(precision_at_k.keys()):
                p_val = precision_at_k[k]
                html += f"""
            <tr>
                <td><strong>Precision @ {k}</strong></td>
                <td>{p_val:.1f}%</td>
                <td>[Percentage of failing tests in the Top {k} recommendations]</td>
            </tr>
        """
