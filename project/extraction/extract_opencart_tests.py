"""
extract_opencart_tests.py

Extract lightweight testcase metadata from OpenCart Selenium test suite.
Scans Java test files under the OpenCart project, extracts @Test methods,
UI selectors, and synthetic metadata, then writes to data/medium_testcases.json.

This script is tailored for OpenCart's test structure and generates the
"medium" dataset for agentic test prioritization.

Usage:
    python extraction/extract_opencart_tests.py /path/to/opencart/tests
"""
...
def main(argv: Optional[List[str]] = None) -> int:
    """Extract OpenCart testcases and write to data/medium_testcases.json."""
    parser = argparse.ArgumentParser(description="Extract OpenCart Selenium test cases")
    parser.add_argument("root", nargs="?", default=".", help="OpenCart project root")
    parser.add_argument("--out", default=None, help="Output JSON path")

    args = parser.parse_args(argv)
    project_root = Path(args.root).resolve()

    logger.info("Scanning OpenCart tests under: %s", project_root)
    tests = scan_java_tests(project_root)

    if not tests:
        logger.warning("No OpenCart tests found")
        return 1

    # Default output: data/medium_testcases.json
    if args.out:
        out_path = Path(args.out)
    else:
        # Write to PBL/project/data/, not apps/opencart/data/
        project_data_dir = Path(__file__).resolve().parent.parent / "data"
        out_path = project_data_dir / "medium_testcases.json"
    write_small_testcases(tests, out_path)
    logger.info("OpenCart medium testcases written to: %s", out_path)
    return 0
